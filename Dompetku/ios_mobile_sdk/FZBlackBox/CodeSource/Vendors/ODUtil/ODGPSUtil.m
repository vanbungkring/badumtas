...
+(void)showMapWithStart:(MKMapView*)mapView ToGo:(NSDecimalNumber*)eLongi and:(NSDecimalNumber*)eLat
{
    if ([ODDeviceUtil deviceSystemVersion]<6.0f) {
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString: [NSString stringWithFormat:@"http://maps.FLASHiZ.com/maps?saddr=%f,%f&daddr=%f,%f"
...
#pragma mark FLASHiZ Maps App

+(BOOL)FLASHiZMapsAppsInstalled
{
    return [[UIApplication sharedApplication] canOpenURL:
            [NSURL URLWithString:@"comFLASHiZmaps://"]];
}

+(void)showMapWithFLASHiZMaps:(MKMapView*)mapView ToGo:(NSDecimalNumber*)eLongi and:(NSDecimalNumber*)eLat
{
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString: [NSString stringWithFormat:@"comFLASHiZmaps://?saddr=%f,%f&daddr=%f,%f"
                                                                      ,mapView.userLocation.location.coordinate.latitude,mapView.userLocation.location.coordinate.longitude,[eLat floatValue],[eLongi floatValue]]]];
}
...
+ (UIActionSheet*)getActionbar:(UIViewController*)ctrl withTitle:(NSString*)title andCancelButtonTitle:(NSString*)cancelButton
{
    ...
    if([self FLASHiZMapsAppsInstalled]){
        haveFLASHiZMaps = true;
    }
    ...
    if (haveOldIos) {
        [tempArray addObject:kActionSheetGPSFLASHiZMaps];
    }
    
    if (!haveOldIos && haveFLASHiZMaps) {
        [tempArray addObject:kActionSheetGPSFLASHiZMaps];
    }
    ...
    actionSheet = [[[UIActionSheet alloc] initWithTitle:title delegate:(id<UIActionSheetDelegate>)ctrl cancelButtonTitle:nil destructiveButtonTitle:[cancelButton capitalizedString] otherButtonTitles:nil]autorelease];
